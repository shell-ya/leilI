...
import com.linkwin.common.annotation.DataScope;
...
public class SysDeptServiceImpl implements ISysDeptService
{
    ...
    @Override
    public SysDept selectOrganByCode(String code) {
        return deptMapper.selectOrganByCode(code);
    }

    @Override
    @DataScope(deptAlias = "d")
    public List<SysDept> selectDeptProvinceList(SysDept dept){
        return deptMapper.selectDeptProvinceList(dept);
    }

    /**
     * 批量删除关联
     * @param organId 角色ID
     * @param productId 需要删除的用户数据ID
     * @return
     */
    @Override
    public int deleteAuthUsers(String organId, String productId)
    {
        return deptMapper.deleteUserRoleInfos(organId, Convert.toStrArray(productId));
    }

    @Transactional
    @Override
    public int saveOrganAndPd(String organCode, String pdCodes) {
        String[] codes = pdCodes.split(",");
        List<SysProductOrgan> list=new ArrayList<>();
        for (String code:codes){
            SysProductOrgan productOrgan=new SysProductOrgan();
            productOrgan.setOrganCode( organCode );
            productOrgan.setProductCode( code );
            list.add(productOrgan);
        }
        if (list.size()>0){
            return deptMapper.batchInsertOrganPd(list);
        }
        return 0;
    }

    public List<SysDept> selectSysDeptListonlyName(SysDept dept){
        return deptMapper.selectSysDeptListonlyName(dept);
    }

    @Override
    public SysDept selectCountry(Long deptId, String agentArea) {
        return deptMapper.selectCountry(deptId,agentArea);
    }
    ...
    public List<Long> selectDeptId(Long id) {
        ...
        return deptMapper.selectDeptIdList(sysDept);
    }
