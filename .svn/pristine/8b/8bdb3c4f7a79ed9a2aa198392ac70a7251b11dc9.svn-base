    ...
    $.extend({
        ...
        table: {
            ...
            exportExcel: function (formId) {
                table.set();
                var english = $.checkLanguage.isEnglish(table.options.modalName);
                $.modal.confirm(
                    english ? "Are you sure you want to export all " + table.options.modalName + "？":
                    ...
                    $.modal.loading( english ? "Exporting data, please wait..." :"正在导出数据，请稍候...");
        ...
        modal: {
            ...
            alert: function (content, type) {
                var isEnglish = $.checkLanguage.isEnglish(content);
                top.layer.alert(content, {
                    icon: $.modal.icon(type),
                    title: isEnglish ? "System Prompt" : "系统提示",
                    btn: isEnglish ? ['confirm'] : ['确认'],
                    btnclass: ['btn btn-primary'],
                });
            },
            // 国际版 弹出提示
            alert: function (language, content, type) {
                top.layer.alert(content, {
                    icon: $.modal.icon(type),
                    title: (language === 'en_US' ? "System Prompt" : "系统提示"),
                    btn: (language === 'en_US' ? ['confirm'] : ['确认']),
                    btnclass: ['btn btn-primary'],
                });
            },
            // 错误提示
            ...
            // 国际版 错误提示
            alertError: function (language, content) {
                $.modal.alert(language, content, modal_status.FAIL);
            },
            // 成功提示
            ...
            // 警告提示
            alertWarning: function (language,content) {
                $.modal.alert(language,content, modal_status.WARNING);
            },
            // 消息提示，重新加载页面
            ...
            confirm: function (content, callBack) {
                ...
                top.layer.confirm(content, {
                    icon: 3,
                    title: isEnglish ? "System Prompt" :  "系统提示",
                    btn: isEnglish ?  ['confirm', 'close'] :['确认', '取消']
                }, function (index) {
                    $.modal.close(index);
                    callBack(true);
                });
            ...
            open: function (title, url, width, height, callback) {
                ...
                btn = $.checkLanguage.isEnglish(title) ? ['confirm', 'close'] : ['确定', '关闭'];
                top.layer.open({
                    type: 2,
            ...
            // 弹出层指定宽度
            openOnlyframe: function (title, url, endfunction, width, height, callback, cancelFunction) {
                // 如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
                ...
                if ($.common.isEmpty(endfunction)) {
                    endfunction = function (index, layero) {
                        var iframeWin = layero.find('iframe')[0];

                        iframeWin.contentWindow.refresh(index, layero);
                    }
                }
                if ($.common.isEmpty(cancelFunction)) {
                    cancelFunction = function (index, layero) {
                        var iframeWin = layero.find('iframe')[0];

                        iframeWin.contentWindow.cancelFunction(index, layero);
                    }
                }
            ...
            openOptions: function (options) {
                ...
                if($.checkLanguage.isEnglish(options.title)){
                    _btn =['<i class="fa fa-check"></i> confirm', '<i class="fa fa-close"></i> close'];
                }else{
                    _btn =['<i class="fa fa-check"></i> 确认', '<i class="fa fa-close"></i> 关闭'];
                }
                // 如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    _width = 'auto';
        ...
        operate: {
            ...
            remove: function (id) {
                table.set();
                var isEnglish = $.checkLanguage.isEnglish(table.options.modalName);
                $.modal.confirm(
                    isEnglish ?
                        "Are you sure you want to delete this " + table.options.modalName + " information ?" :
                    "确定删除该条" + table.options.modalName + "信息吗？", function () {
            ...
            removeAll: function () {
                ...
                if (rows.length == 0) {
                    $.modal.alertWarning(isEnglish ? "Please select at least one record" : "请至少选择一条记录");
                    return;
                }
                // console.log(table.options.modalName)
                $.modal.confirm(
                    isEnglish ?
                        "Are you sure you want to delete the selected " + rows.length + " piece of data?" :
            ...
            removeAllByText: function (text) {
                // var msg;
                // if (/^[\u4e00-\u9fa5]+$/.test(text.trim())) {
                //     // btnText = ['确定', '关闭'];
                //     msg = "请至少选择一条记录";
                // } else {
                //     // btnText = ['confirm', 'close'];
                ...
                $.modal.confirm(
                    isEnglish ?
                    "Are you sure you want to delete the selected " + rows.length + " piece of data?" :
                    "确认要删除选中的" + rows.length + "条数据吗?" , function () {
                    var url = table.options.removeUrl;
                    var data = {"ids": rows.join()};
                    $.operate.submit(url, "post", "json", data);
                });
            ...
            add: function (id) {
                table.set();
                if (/^[\u4e00-\u9fa5]+$/.test(table.options.modalName.trim())) {
            ...
            addTab: function (id) {
                table.set();
                if($.checkLanguage.isEnglish(table.options.modalName)){
            ...
            edit: function (id) {
                ...
                if ($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    if ($.common.isEmpty(row)) {
                        $.modal.alertWarning(english ? "Please select one record." :"请至少选择一条记录");
                        return;
                    }
                    var url = table.options.updateUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.open(english ? "Modify "+ table.options.modalName : "修改 " + table.options.modalName, url);
                } else {
                    $.modal.open(english ? "Modify "+ table.options.modalName : "修改 " + table.options.modalName, $.operate.editUrl(id));
                }
            },

            // 修改信息
            editByPrefix: function (prefix, id) {
                table.set();
                if ($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    var row = $("#" + table.options.id).bootstrapTreeTable('getSelections')[0];
                    if ($.common.isEmpty(row)) {
                    ...
                    var url = table.options.updateUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.open(prefix + table.options.modalName, url);
                } else {
                    $.modal.open(prefix + table.options.modalName, $.operate.editUrl(id));
                }
            },
            // 修改信息，以tab页展现
            editTab: function (id) {
        ...
        common: {
            ...
            // 数据字典转下拉框
            dictToSelectforPD: function (datas, value, name) {
                var actions = [];
                actions.push($.common.sprintf("<select class='form-control' style=\"width:100%;\" name='%s'>", name));
                $.each(datas, function (index, dict) {
                    actions.push($.common.sprintf("<option value='%s'", dict.code));
                    if (dict.code == ('' + value)) {
                        actions.push(' selected');
                    }
                    actions.push($.common.sprintf(">%s</option>", dict.name));
                });
                actions.push('</select>');
                return actions.join('');
            },
            // 数据字典转下拉框
            dictToSelectforPDdisplay: function (datas, value, name) {
                var actions = [];
