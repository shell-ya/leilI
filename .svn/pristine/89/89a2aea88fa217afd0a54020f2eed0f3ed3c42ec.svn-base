...
public class WhstockServiceImpl implements IWhstockService {
    ...
    public List<Whstock> selectWhstockListByWarehouse(Whstock whstock,List<Long> deptIds) {
        return whstockMapper.selectWhstockListByWarehouse(whstock,deptIds);
    }
...
//            throw new RuntimeException("没有该产品批次的库存");
            throw new RuntimeException(MessageUtils.message("error.msg.notstock"));
        }
...
//            throw new RuntimeException("当前仓库该批次产品的 剩余库存不足");
            throw new RuntimeException(MessageUtils.message("error.msg.warehousecurrently"));
        }
...
//                throw new Exception("该产品不存在，产品编号：" + whstock.getProductid());
                throw new Exception(MessageUtils.message("error.msg.productnotexist") + whstock.getProductid());
            }
...
//            throw new RuntimeException("处理异常盘点异常" + e.getCause());
            throw new RuntimeException(MessageUtils.message("error.msg.inventoryhandling") + e.getCause());
        }
...
//        Warehouse.setDeptId(Long.parseLong(targetWHid));
        // TODO: 2023/6/19 仓库表里没有对应仓库 就不设置仓库名称
        Warehouse warehouse = warehouseMapper.selectByCode(targetWHid);
        //如果仓库编号查询不到 使用机构编号查询仓库
        //关单使用customer 做形参传递 targetWHid  customer是机构编码 会存在查不到仓库的情况 使用机构编码查询仓库
        List<Warehouse> warehouseList = warehouseMapper.selectByOrganCode(targetWHid);

        if (!warehouseList.isEmpty() && Objects.isNull(warehouse) ){
...
//                throw new RuntimeException("当前仓库没有该批次产品的库存!!");
                throw new RuntimeException(MessageUtils.message("error.msg.noinventory"));
//                }else{
//                    noPDflag=true;
//
...
//                freezeno = whstock.getFreezeno() == null ? BigDecimal.valueOf(0) : whstock.getFreezeno();
            ...
            if (itq.compareTo(BigDecimal.valueOf(0)) == -1 && uploadType != 2) {
                //如果剩余冻结库存小于0
//                throw new RuntimeException("当前仓库该批次产品的 剩余库存不足");
                throw new RuntimeException(MessageUtils.message("error.msg.inadequatestock"));
            }
